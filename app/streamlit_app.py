...
def make_prediction(patient_data: dict) -> dict:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {"success": False, "error": "Validation Error", "details": error_detail}
        else:
            return {"success": False, "error": response.json().get("detail", "Unknown error")}
    except requests.exceptions.ConnectionError:
        return {"success": False, "error": "Cannot connect to API. Please ensure the API server is running."}
